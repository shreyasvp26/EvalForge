"""Gemini Adapter integration tests — mocked stream sources only."""
    ...
    *,
    ...
    return ExecutionContext(
        ...
        run=RunMetadata(
            ...
            adapter_version_id="adapter-gemini-v1",
...
SAMPLE_STREAM = [
    ...
    json.dumps({"type": "complete", "status": "success", "result": "fixed"}),
]


def test_gemini_happy_path_streaming_and_translation() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield from SAMPLE_STREAM

    sink = RecordingSink()
    result = run_adapter(GeminiAdapter(stream_source=source), _context(), sink)
...
def test_gemini_event_ordering_and_exactly_once() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield from SAMPLE_STREAM
...
def test_gemini_cancellation_during_stream() -> None:
    ...
    result = run_adapter(
        GeminiAdapter(stream_source=source),
...
def test_gemini_timeout_during_stream() -> None:
    ...
    result = run_adapter(
        GeminiAdapter(stream_source=slow_source),
...
def test_gemini_malformed_output_recovery() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield "not-json{{{"
        yield SAMPLE_STREAM[-1]
...
def test_gemini_agent_failed_completion() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield json.dumps(
...
def test_gemini_large_output_emits_artifact() -> None:
    big = "x" * 10_000
...
def test_gemini_sandbox_process_startup_path() -> None:
    stdout = "\n".join(SAMPLE_STREAM)
