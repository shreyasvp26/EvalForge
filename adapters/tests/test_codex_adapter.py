"""Codex Adapter integration tests — mocked stream sources only."""
...
from agent_eval_adapters.codex.parser import parse_stream_line
    ...
    *,
    ...
    return ExecutionContext(
        ...
        run=RunMetadata(
            ...
            adapter_version_id="adapter-codex-v1",
...
SAMPLE_STREAM = [
    ...
    json.dumps({"type": "turn.completed", "status": "success", "result": "fixed"}),
]


def test_codex_happy_path_streaming_and_translation() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield from SAMPLE_STREAM

    sink = RecordingSink()
    result = run_adapter(CodexAdapter(stream_source=source), _context(), sink)
...
def test_codex_event_ordering_and_exactly_once() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield from SAMPLE_STREAM
...
def test_codex_cancellation_during_stream() -> None:
    ...
    result = run_adapter(
        CodexAdapter(stream_source=source),
...
def test_codex_timeout_during_stream() -> None:
    ...
    result = run_adapter(
        CodexAdapter(stream_source=slow_source),
...
def test_codex_malformed_output_recovery() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield "not-json{{{"
        yield SAMPLE_STREAM[-1]
...
def test_codex_agent_failed_completion() -> None:
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield json.dumps(
...
def test_codex_large_output_emits_artifact() -> None:
    big = "x" * 10_000
    ...
    def source(_ctx: ExecutionContext) -> Iterator[str]:
        yield json.dumps(
            {
                "type": "item.completed",
                "item": {"type": "file_edit", "path": "big.txt", "content": big},
            }
        )
...
def test_codex_sandbox_process_startup_path() -> None:
    stdout = "\n".join(SAMPLE_STREAM)
    ...
    assert result.outcome is AdapterOutcome.COMPLETED
    assert exec_port.commands[0][0] == "codex"
    assert "exec" in exec_port.commands[0]
    assert "--json" in exec_port.commands[0]


def test_codex_initialize_requires_prompt() -> None:
    with pytest.raises(AdapterInitializationError):
        run_adapter(CodexAdapter(), _context(prompt=""), RecordingSink())


def test_codex_parser_rejects_non_object() -> None:
