"""Claude Code production Adapter — translation only."""
...
class ClaudeCodeAdapter(BaseAdapter):
    """Observes Claude Code CLI stream-json output inside a Sandbox.

    Supports process startup (via SandboxExecPort), streaming parse, tool /
    file / shell translation hooks (in parser), completion detection, structured
    errors, and graceful cancellation / timeout checks between stream lines.
    """
    ...
    # Test / recovery hook: inject NDJSON lines instead of sandbox exec.
    ...
    def prepare(self, context: ExecutionContext) -> None:
        if not context.prompt.strip() and self.stream_source is None:
            raise AdapterInitializationError(
                "Prompt content is required for Claude Code",
    ...
    def _check_bounds(self, context: ExecutionContext) -> None:
        ...
        if elapsed > context.config.timeout_seconds:
            raise AdapterTimeoutError(
                "Claude Code stream exceeded timeout",
                details={
                    "run_id": context.run.run_id,
