"""Cursor Agent production Adapter — translation only."""
...
class CursorAdapter(BaseAdapter):
    """Observes Cursor Agent CLI NDJSON output inside a Sandbox.

    Supports process startup (via SandboxExecPort), streaming parse, tool /
    file / shell translation, completion detection, structured errors, and
    cancellation / timeout checks between stream lines.
    """
    ...
    def prepare(self, context: ExecutionContext) -> None:
        if not context.prompt.strip() and self.stream_source is None:
            raise AdapterInitializationError(
                "Prompt content is required for Cursor",
    ...
    def _check_bounds(self, context: ExecutionContext) -> None:
        ...
        if elapsed > context.config.timeout_seconds:
            raise AdapterTimeoutError(
                "Cursor stream exceeded timeout",
                details={
                    "run_id": context.run.run_id,
