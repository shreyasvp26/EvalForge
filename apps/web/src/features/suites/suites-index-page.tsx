...
/**
 * Global /suites entry: suites are project-scoped, so guide the user into a project.
 */
export function SuitesIndexPage() {
  const { token } = useAuth();
  const query = useQuery({
    queryKey: [...projectsQueryKey, "list", "suites-index"],
      ...
      <Section className="mt-8">
        {query.isError ? (
          ...
          <EmptyContent
            ...
            description="Fetching projects so you can open suites."
          />
          ...
          <EmptyContent
            ...
            description="Create a project first, then add suites under that project."
            action={
              <Button asChild>
