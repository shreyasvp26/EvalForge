...
def test_create_and_get_grader(client, services, auth_headers) -> None:
    ...
    services.list_graders.execute.return_value = [grader]
    ...
    listed = client.get("/v1/graders", headers=auth_headers)
    assert listed.status_code == 200
    assert listed.json()["count"] == 1


def test_list_agents_and_adapters(client, services, auth_headers) -> None:
    agent = AgentDTO(
        id="agent-1",
        name="Claude",
        description="",
    ...
    services.list_agents.execute.return_value = [agent]
    services.list_adapters.execute.return_value = [adapter]
    services.get_adapter.execute.return_value = adapter

    agents = client.get("/v1/agents", headers=auth_headers)
    assert agents.status_code == 200
    assert agents.json()["items"][0]["name"] == "Claude"

    adapters = client.get("/v1/adapters", headers=auth_headers)
    assert adapters.status_code == 200
    assert adapters.json()["count"] == 1

    got = client.get("/v1/adapters/adapter-1", headers=auth_headers)
    assert got.status_code == 200
    assert got.json()["agent_id"] == "agent-1"
