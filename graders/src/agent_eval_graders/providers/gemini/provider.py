"""Gemini ``JudgeProvider`` — implements the existing rubric port."""
...
from agent_eval_graders.providers.gemini.client import GeminiClient
...
class GeminiJudgeProvider:
    """Production Google Gemini judge. Seed is supported via generationConfig."""

    config: GeminiJudgeConfig
    http_client: httpx.Client | None = None
    _client: GeminiClient | None = field(default=None, init=False, repr=False)

    def __post_init__(self) -> None:
        self._client = GeminiClient(self.config, http_client=self.http_client)
